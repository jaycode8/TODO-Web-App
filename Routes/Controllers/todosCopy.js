        ...
        conn.query(`SELECT * FROM users WHERE userName = "${userName}"`,(err,rows) =>{
            if(err){
                ...
                if(userName == "ADMIN" || userName == "admin" || userName == "Admin"){
                    ...
                    if(rows == ''){
                        conn.query(`INSERT INTO users (userName, phone, userPassword, profile_pic) VALUES ("${userName}","${phone}","${password}","${file.filename}")`, (err,rows)=>{
                            if(err){
                                res.render('error500',{
                                    warning:err.message,
                                    title: err.message,
...
const userAuthentication = (req,res) =>{
    ...
    conn.query(`SELECT * FROM users WHERE userName = "${user}" AND userPassword = "${password}"`,(err,rows,results) =>{
        if(err){
            res.render('error500',{
                warning: err.message,
                title: err.message,
...
const updateUser = (req,res) =>{
    ...
    conn.query(`SELECT * FROM users WHERE user_id = ${userID}`, (err,rows) =>{
        if(err){
            ...
            if(!file){
                conn.query(`UPDATE users SET userName = "${userName}", phone = "${phone}", userPassword = "${password}" WHERE user_id = ${userID}`, (err,rows)=>{
                    if(err){
                        res.render('error500',{
                            warning: err.message,
                            title: err.message,
                ...
                conn.query(`UPDATE users SET userName = "${userName}", phone = "${phone}", userPassword = "${password}", profile_pic = "${file.filename}" WHERE user_id = ${userID}`, (err,rows)=>{
                    if(err){
                        res.render('error500',{
                            warning: err.message,
                            title: err.message,
